            ...
            <div class="x_content">
                <div class="btn-group hidden-xs" id="toolbar" style="padding: 10px 0;">
                    <@shiro.hasPermission name="files">
                        <button id="btn_add" type="button" class="btn btn-default" name="新增图片">
            ...
            $.ajax({
                ...
                success: function (json) {
                    var tpl = '{{#list}}<div class="col-md-55">\n' +
                        ...
                        '                            <img style="width: 100%; display: block;" src="{{fullFilePath}}" onerror="this.alt=\'图片加载失败\'" alt="{{originalFileName}}" name="{{originalFileName}}" />\n' +
                        '                            <div class="vmask">\n' +
                        '                                <p>点击选中</p>\n' +
                        '                                <div class="tools tools-bottom">\n' +
                        '                                    <a href="{{fullFilePath}}" class="file-icon showImage" name="{{filePath}}"><i class="fa fa-eye"></i></a>\n' +
                        '                                    <a href="{{fullFilePath}}" target="_blank" class="file-icon" name="复制地址（打开标签后复制）"><i class="fa fa-link"></i></a>\n' +
                        '                                    <a class="pointer file-icon" data-event="del" data-value="{{id}}" data-storage-type="{{storageType}}" name="删除文件"><i class="fa fa-times"></i></a>\n' +
                        '                                </div>\n' +
                        '                            </div>\n' +
                        '                            <div class="selected-mask">\n' +
                        ...
                        '                        </div>\n' +
                        '                        <div class="caption">\n' +
                        '                            <p><span name="{{originalFileName}}">{{originalFileName}}</span><img src="/assets/images/icons/{{storageType}}.svg" alt="{{storageType}}" name="{{storageType}}"></p>\n' +
                        '                        </div>\n' +
                        '                    </div>\n' +
                        '                </div>{{/list}}';
