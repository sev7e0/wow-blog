    ...
    <div class="col-sm-12 blog-main">
        <div class="blog-header">
            ...
            <div>
                ...
                |
                <a href="mailto:${config.authorEmail}" target="_blank" name="点击给我发邮件" rel="external nofollow"><i class="fa fa fa-envelope fa-fw"></i>邮箱联系</a>
                |
                <a href="${config.weibo}" target="_blank" name="点击查看我的微博" rel="external nofollow"><i class="fa fa fa-weibo fa-fw"></i>${weiboName}</a>
            </div>
        </div>
    </div>
</#macro>
    ...
    <#if config.wxPraiseCode?if_exists || config.zfbPraiseCode?if_exists>
        ...
        <div class="col-sm-12 col-md-12" style="float: initial;">
            <#if config.zfbPraiseCode?if_exists>
                <a href="${config.zfbPraiseCode}" class="showImage" name="支付宝收钱码" rel="external nofollow">
                    <img src="${config.zfbPraiseCode}" alt="支付宝收钱码" class="img-rounded" style="width: 250px;height: auto;">
                </a>
            </#if>
            <#if config.wxPraiseCode?if_exists>
                <a href="${config.wxPraiseCode}" class="showImage" name="微信收钱码" rel="external nofollow">
